...
def sample_uniform_points_in_unit_cube(amount):
    unit_cube_points = np.random.uniform(-1, 1, size=(amount * 2 + 20, 3))

    points_available = unit_cube_points.shape[0]
    if points_available < amount:
        # This is a fallback for the rare case that too few points are inside the cube
        result = np.zeros((amount, 3))
        result[:points_available, :] = unit_cube_points
        result[points_available:, :] = sample_uniform_points_in_unit_cube(amount - points_available)
        return result
    else:
        return unit_cube_points[:amount, :]
